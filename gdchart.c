...
static int GDCImageStringNL(gdImagePtr im, GDC_FONT_T * f, char *ftfont,
                            double ftptsz, double rad, int x, int y, char *str, int clr, GDC_justify_t justify, char **sts)
{
    int retval = 0;
    char *err = NULL;
...
{
    if (((struct YS *) a)->y2 < ((struct YS *) b)->y2)
        return 1;
    if (((struct YS *) a)->y2 > ((struct YS *) b)->y2)
        return -1;
    return 0;
...
static void draw_3d_line(gdImagePtr im, int y0, int x1, int x2, int y1[], int y2[], int xdepth, int ydepth, int num_sets,
                         unsigned long clr[], unsigned long clrshd[])
{
    ...
    for (x = x1 + 1; x <= x2; ++x) {
        ...
        for (i = 0; i < num_sets; ++i) {        /* top */
            ...
            if (x == x1 + 1)
                gdImageLine(im, x - 1, ypts[i].y2, x - 1 + xdepth, ypts[i].y2 - ydepth,
...
static void draw_3d_area(gdImagePtr im, int x1, int x2, int y0, int y1, int y2, int xdepth, int ydepth, int clr, int clrshd)
{
    ...
    if (xdepth || ydepth) {
        ...
        y_intercept = (y1 > y0 && y2 < y0) || (y1 < y0 &&
                                               y2 > y0) ? (int) ((1.0 / ABS(line_slope)) * (float) (ABS(y1 - y0))) + x1 : 0;
        ...
        if (y_intercept)
            gdImageLine(im, y_intercept, y0, y_intercept + xdepth, y0 - ydepth, clrshd);
    ...
    if ((xdepth || ydepth) && (y1 < y0 || y2 < y0)) {   /* and only above y0 */
        ...
        else if (y1 < y0 && y2 > y0)    /* line crosses from above y0 */
...
void GDC_free(GDC_T * gdc)
{
    ...
    for (i = 0; i < gdc->num_sets; i++)
        ns_free(gdc->dlabels[i]);
    ns_free(gdc->dlabels);
    for (i = 0; i < gdc->num_points; i++)
        ns_free(gdc->xlabels[i]);
    ns_free(gdc->xlabels);
    ns_free(gdc->ytitle);
    ns_free(gdc->xtitle);
    ns_free(gdc->ytitle2);
...
int GDC_graph(GDC_T * GDC)
{
    ...
    char do_vol = (GDC->type == GDC_COMBO_HLC_BAR ||
                   ...
                   GDC->type == GDC_3DCOMBO_LINE_AREA || GDC->type == GDC_3DCOMBO_LINE_LINE);
    ...
    char do_bar = (GDC->type == GDC_3DBAR ||
                   GDC->type == GDC_BAR || GDC->type == GDC_3DFLOATINGBAR || GDC->type == GDC_FLOATINGBAR);
    ...
    if (GDC->num_sets <= 0 || GDC->num_points <= 0 || GDC->width <= 0 || GDC->height <= 0) {
        Ns_Log(Error, "num_sets=%d, num_points=%d, width=%d, height=%d", GDC->num_sets, GDC->num_points, GDC->width,
               GDC->height);
        return -1;
    }
    if (GDC->type == GDC_HILOCLOSE ||
        GDC->type == GDC_3DHILOCLOSE ||
        GDC->type == GDC_3DCOMBO_HLC_BAR ||
        GDC->type == GDC_3DCOMBO_HLC_AREA || GDC->type == GDC_COMBO_HLC_BAR || GDC->type == GDC_COMBO_HLC_AREA) {
        if (GDC->num_sets != 4) {
    ...
    for (i = 0; i < num_sets; ++i)
        uvals[i] = GDC->data + i * GDC->num_points;
    if (GDC->thumbnail)
        GDC->grid = FALSE, GDC->xaxis = FALSE, GDC->yaxis = FALSE;
        ...
        for (i = 0; i < num_sets; ++i)
            for (j = 0; j < GDC->num_points; ++j)
                if (uvals[i][j] == GDC_INTERP_VALUE)
                    do_interpolations(GDC->num_points, j, uvals[i]);
        if (do_vol)
            for (j = 0; j < GDC->num_points; ++j)
                if (uvol[j] == GDC_INTERP_VALUE)
                    do_interpolations(GDC->num_points, j, uvol);
    }
    ...
    } else if (GDC->stack_type == GDC_STACK_LAYER)      /* need to walk sideways */
        for (j = 0; j < GDC->num_points; ++j) {
    ...
    if (do_vol) {               /* for now only one combo set allowed */
        ...
        if (GDC->type == GDC_COMBO_LINE_BAR ||
            ...
            GDC->type == GDC_COMBO_HLC_AREA ||
            GDC->type == GDC_3DCOMBO_LINE_BAR ||
            GDC->type == GDC_3DCOMBO_LINE_AREA || GDC->type == GDC_3DCOMBO_HLC_BAR || GDC->type == GDC_3DCOMBO_HLC_AREA) {
            if (vhighest < 0.0)
                vhighest = 0.0;
            else if (vlowest > 0.0)
                vlowest = 0.0;
    ...
    if (GDC->type == GDC_AREA || GDC->type == GDC_LINEAREA ||
        GDC->type == GDC_BAR || GDC->type == GDC_3DBAR || GDC->type == GDC_3DAREA) {
        if (highest < 0.0)
            highest = 0.0;
        else if (lowest > 0.0)
            lowest = 0.0;
    }
    if (GDC->requested_ymin != GDC_NOVALUE && GDC->requested_ymin < lowest)
        lowest = GDC->requested_ymin;
    if (GDC->requested_ymax != GDC_NOVALUE && GDC->requested_ymax > highest)
        highest = GDC->requested_ymax;

    /* ----- graph height and width within the img height width ----- */
    /* grapheight/height is the actual size of the scalable graph */
    {
        int title_hgt =
            GDC->title ? 2 + GDCfnt_sz(GDC->title, GDC->title_size, gdc_title_font, gdc_title_ptsize, 0.0, NULL).h + 2 : 2;
        int xlabel_hgt = 0;
        int xtitle_hgt =
            GDC->xtitle ? 1 + GDCfnt_sz(GDC->xtitle, GDC->xtitle_size, gdc_xtitle_font, gdc_xtitle_ptsize, 0.0,
                                        NULL).h + 1 : 0;
        int ytitle_hgt =
            GDC->ytitle ? 1 + GDCfnt_sz(GDC->ytitle, GDC->ytitle_size, gdc_ytitle_font, gdc_ytitle_ptsize, M_PI / 2.0,
                                        NULL).h + 1 : 0;
        int vtitle_hgt = do_vol &&
            GDC->ytitle2 ? 1 + GDCfnt_sz(GDC->ytitle2, GDC->ytitle_size, gdc_ytitle_font, gdc_ytitle_ptsize, M_PI / 2.0,
                                         NULL).h + 1 : 0;
        int ylabel_wth = 0;
        int vlabel_wth = 0;
        ...
        annote_hgt =
            GDC->annotation.note[0] ? 1 + (1 +
                                           GDCfnt_sz(GDC->annotation.note, GDC->annotation_font_size, gdc_annotation_font,
                                                     gdc_annotation_ptsize, 0.0, NULL).h) + 1 + 2 : 0;
        annote_len =
            GDC->annotation.note[0] ? GDCfnt_sz(GDC->annotation.note, GDC->annotation_font_size, gdc_annotation_font,
                                                gdc_annotation_ptsize, 0.0, NULL).w : 0;
        ...
        if (GDC->xaxis && GDC->xlabels) {
            ...
            for (i = 0; i < GDC->num_points; ++i) {
                ...
                if (!GDC->xlabel_ctl || (GDC->xlabel_ctl && GDC->xlabel_ctl[i])) {
                    char *sts;
                    GDC_FONT_SZ_T lftsz =
                        GDCfnt_sz(GDC->xlabels[i], GDC->xaxisfont_size, gdc_xaxis_font, gdc_xaxis_ptsize, gdc_xaxis_rad,
                                  &sts);
                    if (gdc_xaxis_rad == M_PI / 2.0 || (sts && *sts)) {
                        len = lftsz.w;
                        wdth = lftsz.h;
                    } else if (gdc_xaxis_rad == 0.0) {  /* protect /0 *//* reverse when horiz. */
                        len = lftsz.h;
                        wdth = lftsz.w;
        ...
        if (GDC->hard_size && GDC->hard_grapheight)
            grapheight = GDC->hard_grapheight;
        ...
        /* ----- y labels intervals ----- */
        {
            ...
            max_num_ylbls =
                grapheight / (3 +
                              GDC_fontc[GDC->yaxisfont_size == GDC_TINY ? GDC->yaxisfont_size + 1 : GDC->yaxisfont_size].h);
            if (max_num_ylbls < 3) {
            ...
            ylbl_interval = GDC->requested_yinterval != GDC_NOVALUE &&
                GDC->requested_yinterval > ypoints[i - 1] ? GDC->requested_yinterval : ypoints[i - 1];
            ...
            do {
                if (GDC->yaxis) {
                    ...
                    lbl_len =
                        ...
                        1.0 ?
                        strlen(price_to_str
                               (pp, tmp_highest, &nmrtr, &dmntr, &whole,
                                do_ylbl_fractions ? NULL : GDC->ylabel_fmt)) : strlen(foo);
        ...
        graphwidth =
            GDC->width - (((GDC->hard_size && GDC->hard_xorig) ? GDC->hard_xorig : (ytitle_hgt + ylabel_wth + ytics)) +
                          vtics + vtitle_hgt + vlabel_wth + xdepth_3Dtotal);
        if (GDC->hard_size && GDC->hard_graphwidth)
            graphwidth = GDC->hard_graphwidth;
    ...
    if (!GDC->image)
        GDC->image = gdImageCreate(GDC->width, GDC->height);
    ...
    if (GDC->annotation.note[0])
        AnnoteColor = clrallocate(GDC->image, GDC->annotation.color);
        ...
        if (in) {
            ...
            if ((bg_img = gdImageCreateFromPng(in))) {
                ...
                if (gdImageSX(bg_img) > GDC->width || gdImageSY(bg_img) > GDC->height) {        /*  [and center] */
                    gdImageCopyResized(GDC->image, bg_img, bgxpos, bgypos, 0, 0, GDC->width, GDC->height, GDC->width,
                                       GDC->height);
                } else          /* just center */
    ...
    for (j = 0; j < num_sets; ++j) {
        for (i = 0; i < GDC->num_points; ++i)
            if (GDC->ExtColor) {
                ...
                if (threeD)
                    ExtColorShd[j][i] = clrshdallocate(GDC->image, ext_clr);
            } else if (GDC->SetColor) {
                unsigned long set_clr = GDC->SetColor[j];
                ExtColor[j][i] = clrallocate(GDC->image, set_clr);
                if (threeD)
                    ExtColorShd[j][i] = clrshdallocate(GDC->image, set_clr);
            } else {
                ExtColor[j][i] = PlotColor;
                if (threeD)
                    ExtColorShd[j][i] = clrshdallocate(GDC->image, GDC->PlotColor);
            }
    }

    if (GDC->transparent_bg)
        gdImageColorTransparent(GDC->image, BGColor);
    ...
    if (GDC->title) {
        ...
        GDCImageStringNL(GDC->image, &GDC_fontc[GDC->title_size], gdc_title_font, gdc_title_ptsize, 0.0,
                         GDC->width / 2 - tftsz.w / 2, 1, GDC->title, titlecolor, GDC_JUSTIFY_CENTER, NULL);
    }
    if (GDC->xtitle) {
        ...
        GDCImageStringNL(GDC->image, &GDC_fontc[GDC->xtitle_size], gdc_xtitle_font, gdc_xtitle_ptsize, 0.0,
                         ...
                         GDC->height - 1 - xtftsz.h - 1, GDC->xtitle, titlecolor, GDC_JUSTIFY_CENTER, NULL);
    }
    if (GDC->grid_on_top)
        goto gdcData;
  ...
  gdcGrid:
    ...
    if (GDC->grid || GDC->ticks || GDC->yaxis) {        /* grid lines & y label(s) */
        ...
        for (i = -1; i <= 1; i += 2) {  /* -1,1 */
            ...
            do {
                ...
                if (GDC->grid || GDC->ticks) {
                    ...
                    y1 = PY(tmp_y);
                    if (GDC->ticks)
                        gdImageLine(GDC->image, x1 - 2, y1, x1, y1, GridColor);
                    if (GDC->grid) {
                        setno = set_depth;
                        x2 = PX(0);
                                          ...
                                          PX(0) - 2 - strlen(whole) * GDC_fontc[GDC->yaxisfont_size].w - ((!all_whole) ?
                                                                                                          (strlen(nmrtr) *
                                                                                                           ...
                                                                                                           GDC_fontc[GDC->
                                                                                                                     ...
                                                                                                                     1].
                                                                                                           w) : 1),
                                          PY(tmp_y) - GDC_fontc[GDC->yaxisfont_size].h / 2, (unsigned char *) whole,
                                          labelcolor);
                        }
                        if (n) {
                            gdImageString(GDC->image,
                                          ...
                                          (unsigned char *) nmrtr, labelcolor);
                            gdImageString(GDC->image,
                                          GDC_fontc[GDC->yaxisfont_size].f,
                            ...
                            gdImageString(GDC->image,
                                          ...
                                          (unsigned char *) dmntr, labelcolor);
                        }
                    } else
                        gdImageString(GDC->image,
                                      GDC_fontc[GDC->yaxisfont_size].f,
                                      ...
                                      PY(tmp_y) - GDC_fontc[GDC->yaxisfont_size].h / 2,
                                      (unsigned char *) ylbl_str, labelcolor);

                    if (do_vol && GDC->yaxis2) {
                        char vylbl[16];
                        /* opposite of PV(y) */
                        sprintf(vylbl, GDC->ylabel2_fmt ? GDC->ylabel2_fmt : "%.0f",
                                ((float) (PY(tmp_y) + (setno * ydepth_3D) - vyorig)) / vyscl);
                        setno = set_depth;
                        if (GDC->ticks)
                            gdImageLine(GDC->image,
                                        PX(GDC->num_points - 1 + (do_bar ? 2 : 0)),
                                        PY(tmp_y), PX(GDC->num_points - 1 + (do_bar ? 2 : 0)) + 3, PY(tmp_y), GridColor);
                        if (atof(vylbl) == 0.0)
                            strcpy(vylbl, "0");
                        gdImageString(GDC->image,
                                      GDC_fontc[GDC->yaxisfont_size].f,
                                      ...
                                      PY(tmp_y) - GDC_fontc[GDC->yaxisfont_size].h / 2,
                                      (unsigned char *) vylbl, label2color);
                        setno = 0;
                    }
                }
        ...
        if (do_vol && GDC->ytitle2) {
            ...
            GDCImageStringNL(GDC->image, &GDC_fontc[GDC->ytitle_size],
                             ...
                             yorig / 2 + ytftsz.w / 2, GDC->ytitle2, titlecolor, GDC_JUSTIFY_CENTER, NULL);
        }
        ...
        if (GDC->yaxis && GDC->ytitle) {
            ...
            GDCImageStringNL(GDC->image, &GDC_fontc[GDC->ytitle_size],
                             gdc_ytitle_font,
                             gdc_ytitle_ptsize,
                             M_PI / 2.0, 1, yorig / 2 + ytftsz.w / 2, GDC->ytitle, titlecolor, GDC_JUSTIFY_CENTER, NULL);
        }
    }
    ...
    if ((GDC->grid || GDC->Shelf) &&
        ((lowest < 0.0 && highest > 0.0) || ((lowest == 0.0 || highest == 0.0) && !(GDC->border & GDC_BORDER_X)))) {
        int x1, x2, y1, y2;
        /* tics */
        x1 = PX(0);
        y1 = PY(0);
        if (GDC->ticks)
            gdImageLine(GDC->image, x1 - 2, y1, x1, y1, LineColor);
        setno = set_depth;
        x2 = PX(0);
        y2 = PY(0);             /* w/ new setno */
    ...
    if (GDC->grid || GDC->xaxis) {
        ...
        for (i = 0; i < GDC->num_points + (do_bar ? 2 : 0); ++i) {
            ...
            if ((!GDC->xlabel_ctl && ((i % (1 + GDC->num_points / num_xlbls) == 0) || num_xlbls >= GDC->num_points ||
                                      GDC->xlabel_spacing == SHRT_MAX)) || (GDC->xlabel_ctl && xi >= 0 &&
                                                                            *(GDC->xlabel_ctl + xi))) {
                ...
                if (!do_bar || (i > 0 && xi < GDC->num_points))
                    if (GDC->xaxis && GDC->xlabels && GDC->xlabels[xi] && *(GDC->xlabels[xi])) {
                        char *sts;
                        GDC_FONT_SZ_T lftsz =
                            GDCfnt_sz(GDC->xlabels[xi], GDC->xaxisfont_size, gdc_xaxis_font, gdc_xaxis_ptsize, gdc_xaxis_rad,
                                      &sts);
                        if (gdc_xaxis_rad == M_PI / 2.0 || (sts && *sts)) {
                            GDCImageStringNL(GDC->image,
                                             ...
                                             GDC->xlabels[xi], labelcolor, GDC_JUSTIFY_RIGHT, NULL);
                        } else if (gdc_xaxis_rad == 0.0)
                            GDCImageStringNL(GDC->image,
                                             &GDC_fontc[GDC->xaxisfont_size],
                                             gdc_xaxis_font,
                                             gdc_xaxis_ptsize, 0.0,
                                             PX(i) - 1 - (lftsz.w / 2),
                                             PY(lowest) + 2 + 1, GDC->xlabels[xi], labelcolor, GDC_JUSTIFY_CENTER, NULL);
                        else
                            GDCImageStringNL(GDC->image,
                                             &GDC_fontc[GDC->xaxisfont_size],
                                             gdc_xaxis_font,
                                             ...
                                             labelcolor, GDC_JUSTIFY_RIGHT, NULL);
                    }
            }
            ...
            if (i > 0) {
                ...
                } else if (GDC->grid > GDC_TICK_NONE) {
                    int k;
                    int xt;
                    int prevx = PX(i - 1);
                ...
                if (GDC->ticks == GDC_TICK_POINTS) {    /* --- TICKS --- */
                    if (GDC->ticks)
                        gdImageLine(GDC->image, x1, y1, x1, y1 + 2, GridColor);
                } else if (GDC->ticks > GDC_TICK_NONE) {
                    int k;
                    int xt;
                    int prevx = PX(i - 1);
                    ...
                    for (k = 0, xt = prevx + intrv_dist; k < GDC->ticks && xt < x1; ++k, xt += intrv_dist)
                        if (GDC->ticks)
                            gdImageLine(GDC->image, xt, y1, xt, y1 + 2, GridColor);
                }
            }
        }
    }
    ...
    if (do_vol) {
        ...
        if (GDC->type == GDC_COMBO_HLC_BAR ||
            GDC->type == GDC_COMBO_LINE_BAR || GDC->type == GDC_3DCOMBO_LINE_BAR || GDC->type == GDC_3DCOMBO_HLC_BAR) {
            if (uvol[0] != GDC_NOVALUE)
                draw_3d_bar(GDC->image, PX(0), PX(0) + hlf_barwdth, PV(0), PV(uvol[0]), 0, 0, ExtVolColor[0],
                            ExtVolColor[0]);
            for (i = 1; i < GDC->num_points - 1; ++i)
                if (uvol[i] != GDC_NOVALUE)
                    draw_3d_bar(GDC->image, PX(i) - hlf_barwdth, PX(i) + hlf_barwdth, PV(0), PV(uvol[i]), 0, 0,
                                ExtVolColor[i], ExtVolColor[i]);
            if (uvol[i] != GDC_NOVALUE)
                draw_3d_bar(GDC->image, PX(i) - hlf_barwdth, PX(i), PV(0), PV(uvol[i]), 0, 0, ExtVolColor[i],
                            ExtVolColor[i]);
        } else if (GDC->type == GDC_COMBO_HLC_AREA || GDC->type == GDC_COMBO_LINE_AREA || GDC->type == GDC_3DCOMBO_LINE_AREA
                   || GDC->type == GDC_3DCOMBO_HLC_AREA) {
            for (i = 1; i < GDC->num_points; ++i)
                if (uvol[i - 1] != GDC_NOVALUE && uvol[i] != GDC_NOVALUE)
                    draw_3d_area(GDC->image, PX(i - 1), PX(i), PV(0), PV(uvol[i - 1]), PV(uvol[i]), 0, 0, ExtVolColor[i],
                                 ExtVolColor[i]);
        } else if (GDC->type == GDC_COMBO_LINE_LINE || GDC->type == GDC_3DCOMBO_LINE_LINE) {
            for (i = 1; i < GDC->num_points; ++i)
                if (uvol[i - 1] != GDC_NOVALUE && uvol[i] != GDC_NOVALUE)
    ...
    if (GDC->annotation.note[0] && threeD) {    /* back half of annotation line */
        int x1 = PX(GDC->annotation.point + (do_bar ? 1 : 0)), y1 = PY(lowest);
        setno = set_depth;
        gdImageLine(GDC->image, x1, y1, PX(GDC->annotation.point + (do_bar ? 1 : 0)), PY(lowest), AnnoteColor);
        gdImageLine(GDC->image,
                    PX(GDC->annotation.point + (do_bar ? 1 : 0)),
                    PY(lowest), PX(GDC->annotation.point + (do_bar ? 1 : 0)), PY(highest) - 2, AnnoteColor);
        setno = 0;
    }
    if (GDC->grid_on_top)
        goto gdcFinish;
  ...
  gdcData:
                ...
                for (i = 0; i < GDC->num_points; ++i)
                    if (uvals[setno][i] != GDC_NOVALUE)
                        draw_3d_bar(GDC->image,
                                    ...
                                    PY(uvals[setno][i]),
                                    xdepth_3D,
                                    ydepth_3D, ExtColor[setno][i], threeD ? ExtColorShd[setno][i] : ExtColor[setno][i]);
            setno = 0;
            break;
                ...
                for (i = 0; i < GDC->num_points; ++i) {
                    ...
                    for (j = 0; j < k; ++j) {
                        draw_3d_bar(GDC->image,
                                    ...
                                    PX(i + (do_bar ? 1 : 0)) + hlf_barwdth,
                                    PY(barset[j].y1),
                                    PY(barset[j].y2), xdepth_3D, ydepth_3D, barset[j].clr, barset[j].shclr);
                    }
                }
            }
            break;
                    ...
                    for (j = 0; j < num_sets; ++j)
                        if (uvals[j][i] != GDC_NOVALUE)
                            draw_3d_bar(GDC->image,
                                        ...
                                        PY(0),
                                        PY(uvals[j][i]),
                                        xdepth_3D, ydepth_3D, ExtColor[j][i], threeD ? ExtColorShd[j][i] : ExtColor[j][i]);
            }
            break;
        }
        break;
    ...
    case GDC_FLOATINGBAR:
        ...
        case GDC_STACK_DEPTH:
            for (i = 0; i < GDC->num_points; ++i)
                if (uvals[0][i] != GDC_NOVALUE && uvals[1][i] != GDC_NOVALUE && uvals[1][i] > uvals[0][i])
                    draw_3d_bar(GDC->image,
                                PX(i + (do_bar ? 1 : 0)) - hlf_barwdth,
                                PX(i + (do_bar ? 1 : 0)) + hlf_barwdth,
                                PY(uvals[0][i]),
    ...
    case GDC_LINEAREA:
        ...
        for (i = 1; i < GDC->num_points; ++i)
            if (uvals[0][i - 1] != GDC_NOVALUE && uvals[0][i] != GDC_NOVALUE)
                draw_3d_area(GDC->image,
                             PX(i - 1),
                             PX(i), PY(0), PY(uvals[0][i - 1]), PY(uvals[0][i]), 0, 0, ExtColor[0][i], ExtColor[0][i]);
        // Draw canvas around the area
        for (i = 1; i < GDC->num_points; ++i)
            if (uvals[0][i - 1] != GDC_NOVALUE && uvals[0][i] != GDC_NOVALUE) {
                gdImageLine(GDC->image, PX(i - 1), PY(uvals[0][i - 1]), PX(i), PY(uvals[0][i]), ExtColor[0][i]);
                gdImageLine(GDC->image, PX(i - 1), PY(uvals[0][i - 1]) + 1, PX(i), PY(uvals[0][i]) + 1, GridColor);
            } else {
                if (uvals[0][i - 1] != GDC_NOVALUE)
                    gdImageSetPixel(GDC->image, PX(i - 1), PY(uvals[0][i - 1]), GridColor);
                if (uvals[0][i] != GDC_NOVALUE)
                    gdImageSetPixel(GDC->image, PX(i), PY(uvals[0][i]), GridColor);
            }
        // All other data sets just line graphs
        for (j = num_sets - 1; j > 0; --j)
            for (i = 1; i < GDC->num_points; ++i)
                if (uvals[j][i - 1] != GDC_NOVALUE && uvals[j][i] != GDC_NOVALUE) {
                    gdImageLine(GDC->image, PX(i - 1), PY(uvals[j][i - 1]), PX(i), PY(uvals[j][i]), ExtColor[j][i]);
                    gdImageLine(GDC->image, PX(i - 1), PY(uvals[j][i - 1]) + 1, PX(i), PY(uvals[j][i]) + 1, ExtColor[j][i]);
                } else {
                    if (uvals[j][i - 1] != GDC_NOVALUE)
    ...
    case GDC_3DCOMBO_LINE_LINE:{
            ...
            for (i = 1; i < GDC->num_points; ++i) {
                if (GDC->stack_type == GDC_STACK_DEPTH) {
                    for (j = num_sets - 1; j >= 0; --j)
                        if (uvals[j][i - 1] != GDC_NOVALUE && uvals[j][i] != GDC_NOVALUE) {
                            ...
                            draw_3d_line(GDC->image,
                                         ...
                                         PX(i),
                                         &(y1[j]),
                                         &(y2[j]), xdepth_3D, ydepth_3D, 1, &(ExtColor[j][i]), &(ExtColorShd[j][i]));
                            setno = 0;
                        }
                } else if (GDC->stack_type == GDC_STACK_BESIDE || GDC->stack_type == GDC_STACK_SUM) {
                    int set;
                    ...
                    draw_3d_line(GDC->image, PY(0), PX(i - 1), PX(i), y1, y2, xdepth_3D, ydepth_3D, set, clr, clrshd);
                }
            }
        }
        break;
                ...
                if (uvals[CLOSESET][i - 1] != GDC_NOVALUE) {
                    if ((GDC->HLC_style & GDC_HLC_I_CAP) && uvals[LOWSET][i - 1] != GDC_NOVALUE) {
                        SET_3D_POLY(poly,
                                    ...
                                    PX(i - 1) + hlf_hlccapwdth,
                                    PY(uvals[LOWSET][i - 1]), PY(uvals[LOWSET][i - 1]), xdepth_3D, ydepth_3D);
                    ...
                    if (uvals[LOWSET][i - 1] != GDC_NOVALUE) {  /* bottom 'half' */
                        SET_3D_POLY(poly,
                                    PX(i - 1),
                                    PX(i - 1), PY(uvals[LOWSET][i - 1]), PY(uvals[CLOSESET][i - 1]), xdepth_3D, ydepth_3D);
                        gdImageFilledPolygon(GDC->image, poly, 4, ExtColor[LOWSET][i - 1]);
                        gdImagePolygon(GDC->image, poly, 4, ExtColorShd[LOWSET][i - 1]);
                    }
                    if (uvals[HIGHSET][i - 1] != GDC_NOVALUE) { /* top 'half' */
                        SET_3D_POLY(poly,
                                    PX(i - 1),
                                    PX(i - 1), PY(uvals[CLOSESET][i - 1]), PY(uvals[HIGHSET][i - 1]), xdepth_3D, ydepth_3D);
                    ...
                    gdImageLine(GDC->image,
                                ...
                                PX(i - 1) + xdepth_3D, PY(uvals[CLOSESET][i - 1]) - ydepth_3D, ExtColorShd[CLOSESET][i - 1]);
                    /* top half 'I' */
                    if (!((GDC->HLC_style & GDC_HLC_DIAMOND) &&
                        ...
                        if (GDC->HLC_style & GDC_HLC_I_CAP) {
                            SET_3D_POLY(poly,
                                        PX(i - 1) - hlf_hlccapwdth,
                                        PX(i - 1) + hlf_hlccapwdth,
                                        PY(uvals[HIGHSET][i - 1]), PY(uvals[HIGHSET][i - 1]), xdepth_3D, ydepth_3D);
                            gdImageFilledPolygon(GDC->image, poly, 4, ExtColor[HIGHSET][i - 1]);
                            gdImagePolygon(GDC->image, poly, 4, ExtColorShd[HIGHSET][i - 1]);
                        }
                    if (i < GDC->num_points && uvals[CLOSESET][i] != GDC_NOVALUE) {
                        if (GDC->HLC_style & GDC_HLC_CLOSE_CONNECTED) { /* line from prev close */
                            SET_3D_POLY(poly,
                                        PX(i - 1),
                                        PX(i), PY(uvals[CLOSESET][i - 1]), PY(uvals[CLOSESET][i - 1]), xdepth_3D, ydepth_3D);
                            gdImageFilledPolygon(GDC->image, poly, 4, ExtColor[CLOSESET][i]);
                            gdImagePolygon(GDC->image, poly, 4, ExtColorShd[CLOSESET][i]);
                        } else if (GDC->HLC_style & GDC_HLC_CONNECTING) {       /* thin connecting line */
                            int y1 = PY(uvals[CLOSESET][i - 1]), y2 = PY(uvals[CLOSESET][i]);
                            draw_3d_line(GDC->image, PY(0), PX(i - 1), PX(i), &y1, &y2, /* rem only 1 set */
                                         xdepth_3D, ydepth_3D, 1, &(ExtColor[CLOSESET][i]), &(ExtColorShd[CLOSESET][i]));
                            /* edge font of it */
                            gdImageLine(GDC->image,
                                        PX(i - 1),
                                        PY(uvals[CLOSESET][i - 1]), PX(i), PY(uvals[CLOSESET][i]), ExtColorShd[CLOSESET][i]);
                        }
                        /* top half 'I' again */
                        if (PY(uvals[CLOSESET][i - 1]) <= PY(uvals[CLOSESET][i]) && uvals[HIGHSET][i - 1] != GDC_NOVALUE)
                            if (GDC->HLC_style & GDC_HLC_I_CAP) {
                                SET_3D_POLY(poly,
                                            PX(i - 1) - hlf_hlccapwdth,
                                            PX(i - 1) + hlf_hlccapwdth,
                    ...
                    if (GDC->HLC_style & GDC_HLC_DIAMOND) {     /* front */
                        ...
                        SET_3D_POLY(poly,
                                    ...
                                    PY(uvals[CLOSESET][i - 1]), xdepth_3D, ydepth_3D);
        ...
        for (i = 0; i < GDC->num_points; ++i)
            if (uvals[CLOSESET][i] != GDC_NOVALUE) {    /* all HLC have vert line */
                if (uvals[LOWSET][i] != GDC_NOVALUE)
                    gdImageLine(GDC->image, PX(i), PY(uvals[CLOSESET][i]), PX(i), PY(uvals[LOWSET][i]), ExtColor[LOWSET][i]);
                if (uvals[HIGHSET][i] != GDC_NOVALUE)
                    gdImageLine(GDC->image,
                                PX(i), PY(uvals[HIGHSET][i]), PX(i), PY(uvals[CLOSESET][i]), ExtColor[HIGHSET][i]);
                if (GDC->HLC_style & GDC_HLC_I_CAP) {
                    if (uvals[LOWSET][i] != GDC_NOVALUE)
                        gdImageLine(GDC->image,
                                    PX(i) - hlf_hlccapwdth,
                                    PY(uvals[LOWSET][i]), PX(i) + hlf_hlccapwdth, PY(uvals[LOWSET][i]), ExtColor[LOWSET][i]);
                    if (uvals[HIGHSET][i] != GDC_NOVALUE)
                        gdImageLine(GDC->image,
                                    PX(i) - hlf_hlccapwdth,
                                    PY(uvals[HIGHSET][i]),
                                    PX(i) + hlf_hlccapwdth, PY(uvals[HIGHSET][i]), ExtColor[HIGHSET][i]);
                }
                if (GDC->HLC_style & GDC_HLC_DIAMOND) {
                    gdPoint cd[4];
        ...
        for (i = 1; i < GDC->num_points; ++i)
            if (uvals[CLOSESET][i - 1] != GDC_NOVALUE && uvals[CLOSESET][i] != GDC_NOVALUE) {
                if (GDC->HLC_style & GDC_HLC_CLOSE_CONNECTED)   /* line from prev close */
                    gdImageLine(GDC->image,
                                PX(i - 1),
                                PY(uvals[CLOSESET][i - 1]), PX(i), PY(uvals[CLOSESET][i - 1]), ExtColor[CLOSESET][i]);
                else if (GDC->HLC_style & GDC_HLC_CONNECTING)   /* thin connecting line */
                    gdImageLine(GDC->image,
                                PX(i - 1), PY(uvals[CLOSESET][i - 1]), PX(i), PY(uvals[CLOSESET][i]), ExtColor[CLOSESET][i]);
            }
        break;
    }
    if (GDC->grid_on_top)
  ...
  gdcFinish:
    ...
    if (GDC->scatter) {
        ...
        for (i = 0; i < GDC->num_scatter_pts; ++i) {
            ...
            int scat_x = PX((GDC->scatter + i)->point + (do_bar ? 1 : 0)), scat_y = PY((GDC->scatter + i)->val);

            if ((GDC->scatter + i)->point >= GDC->num_points || (GDC->scatter + i)->point < 0)
                continue;
            ...
            case GDC_SCATTER_CIRCLE:{
                    ...
                    if (!do_bar) {
                        if ((GDC->scatter + i)->point == 0) {
                            ...
                            e = 270 + 180;
                            fo = 1;
                        } else if ((GDC->scatter + i)->point == GDC->num_points - 1) {
                            s = 90;
                            e = 90 + 180;
                            fo = -1;
                        }
                    ...
                    if (fo)
                        gdImageLine(GDC->image, scat_x, scat_y + hlf_scatterwdth, scat_x, scat_y - hlf_scatterwdth, uc);
                    ...
                    if (fo)
                        gdImageLine(GDC->image, scat_x, scat_y + hlf_scatterwdth, scat_x, scat_y - hlf_scatterwdth,
                                    scatter_clr[i]);
    ...
    if (GDC->border) {
        ...
        if (GDC->border == GDC_BORDER_ALL || (GDC->border & GDC_BORDER_Y2))
            gdImageLine(GDC->image, PX(GDC->num_points - 1 + (do_bar ? 2 : 0)), PY(lowest),
                        PX(GDC->num_points - 1 + (do_bar ? 2 : 0)), PY(highest), LineColor);
        setno = 0;
    }
    ...
    if (GDC->annotation.note[0]) {      /* front half of annotation line */
        int x1 = PX(GDC->annotation.point + (do_bar ? 1 : 0)), y1 = PY(highest);
        int x2;
        /* front line */
        ...
        if (*(GDC->annotation.note)) {  /* any note? */
            if (GDC->annotation.point >= (GDC->num_points / 2)) {       /* note to the left */
                gdImageLine(GDC->image, x2, PY(highest) - 2, x2 - annote_hgt / 2, PY(highest) - 2 - annote_hgt / 2,
                            AnnoteColor);
                GDCImageStringNL(GDC->image, &GDC_fontc[GDC->annotation_font_size], gdc_annotation_font,
                                 gdc_annotation_ptsize, 0.0, x2 - annote_hgt / 2 - 1 - annote_len - 1,
                                 PY(highest) - annote_hgt + 1, GDC->annotation.note, AnnoteColor, GDC_JUSTIFY_RIGHT, NULL);
            } else {            /* note to right */
                gdImageLine(GDC->image, x2, PY(highest) - 2, x2 + annote_hgt / 2, PY(highest) - 2 - annote_hgt / 2,
                            AnnoteColor);
                GDCImageStringNL(GDC->image, &GDC_fontc[GDC->annotation_font_size], gdc_annotation_font,
                                 gdc_annotation_ptsize, 0.0, x2 + annote_hgt / 2 + 1 + 1, PY(highest) - annote_hgt + 1,
                                 GDC->annotation.note, AnnoteColor, GDC_JUSTIFY_LEFT, NULL);
            }
        }
        setno = 0;
    }
