...
static int GDCCmd(ClientData arg, Tcl_Interp * interp, int objc, Tcl_Obj * CONST objv[])
{
    ...
    if (cmd > cmdCreate) {
        ...
        for (gdc = gdcList; gdc; gdc = gdc->next)
            if (gdc->id == i)
                break;
    ...
    if (gdcDebug)
        Ns_Log(Debug, "ns_gdchart[%d]: %s %s %s", getpid(), Tcl_GetString(objv[1]), Tcl_GetString(objv[2]),
    ...
    case cmdSet:
        for (i = cmd == cmdCreate ? 2 : 3; i < objc - 1; i += 2) {
            if (Tcl_GetIndexFromObj(interp, objv[i], Opts, "subcmd", TCL_EXACT, (int *) &opt) != TCL_OK)
                return TCL_ERROR;
            val = Tcl_GetString(objv[i + 1]);
            switch (opt) {
            case optType:
                if (Tcl_GetIndexFromObj(interp, objv[i + 1], Types, "type", TCL_EXACT, (int *) &gdc->type) != TCL_OK)
                    return TCL_ERROR;
                break;
            case optTitle:
                ns_free(gdc->title);
            ...
            case optReqYInterval:
                ...
                break;
            case optImageType:
                if (!strcasecmp(val, "png"))
                    gdc->image_type = GDC_PNG;
                else if (!strcasecmp(val, "jpeg"))
                    gdc->image_type = GDC_JPEG;
                else if (!strcasecmp(val, "wbmp"))
                    gdc->image_type = GDC_WBMP;
                break;
            case optBgImage:
                ns_free(gdc->BGImage);
            ...
            case optShelf:
                ...
                break;
            case optStackType:
                if (!strcasecmp(val, "depth"))
                    gdc->stack_type = GDC_STACK_DEPTH;
                else if (!strcasecmp(val, "sum"))
                    gdc->stack_type = GDC_STACK_SUM;
                else if (!strcasecmp(val, "beside"))
                    gdc->stack_type = GDC_STACK_BESIDE;
                else if (!strcasecmp(val, "layer"))
                    gdc->stack_type = GDC_STACK_LAYER;
                break;
            case optYLabelFmt:
            ...
            case optHLCStyle:
                gdc->HLC_style = 0;
                if (strstr(val, "diamond"))
                    gdc->HLC_style |= GDC_HLC_DIAMOND;
                if (strstr(val, "closeconected"))
                    gdc->HLC_style |= GDC_HLC_CLOSE_CONNECTED;
                if (strstr(val, "conecting"))
                    gdc->HLC_style |= GDC_HLC_CONNECTING;
                if (strstr(val, "icap"))
                    gdc->HLC_style |= GDC_HLC_I_CAP;
                break;
            case optHLCCapWidth:
    ...
    case cmdDestroy:
        Ns_MutexLock(&gdcMutex);
        if (gdc->prev)
            gdc->prev->next = gdc->next;
        if (gdc->next)
            gdc->next->prev = gdc->prev;
        if (gdc == gdcList)
            gdcList = gdc->next;
        Ns_MutexUnlock(&gdcMutex);
        GDC_free(gdc);
    ...
    case cmdSetData:
        ...
        for (i = 0, j = 0; i < argc && j < gdc->num_points; i++, j++)
            *(gdc->data + (gdc->num_sets - 1) * gdc->num_points + j) = *(val =
                                                                         Tcl_GetString(argv[i])) ==
                '*' ? GDC_NOVALUE : atof(val);
        break;
    }
    return TCL_OK;
}
